...
//System.out.println("chad1>type="+type);
		if (type.startsWith("["))
		{
			sourceBuf.append(smalltalkToDirectlyInvokeJavaMethodStackAlreadyPrepared(SmallTalkGeneratorOfAdaptorOfAJavaClass.class, "converToArrayOfObjects"));
	...
	public static String smalltalkToCallJavaMethod(boolean methodIsStatic, Class<?> returnType,String smalltalkCodeToPullAnswerFromStack,Class<?> declaringClass,String smalltalkReceiverToStackCode, String methodName, Class<?>[] parameterTypes,String ... smalltalkCodeToPutArgumentsOnStack  )
	{
		...
		if (!methodIsVoid && !wantsToReturnTheSmallTalkObject)
		{
			...
			sourceBuf.append("  JVM invokeStatic: 'st/redline/core/SmallTalkGeneratorOfAdaptorOfAJavaClass' method:'" + smallTalkObjectForJavaValueMethod.getName() + "' matching: '(" + javaArgsSignature(smallTalkObjectForJavaValueMethod.getParameterTypes()) + ")Lst/redline/core/PrimObject;'.\n");
		}
	...
	*/
	...
	private static Class<?> classForName(String className)
	{
		...
		{
			throw new IllegalStateException(className + " fail to be recognized as a valid java class");
		}
	}
	...
	static public Object[] converToArrayOfObjects(Object toConvert)
	{
		if( Object[].class.isAssignableFrom(toConvert.getClass())) { return (Object[]) toConvert;}
